...
class CrashDetection {
    ...
    async onAliveStateChange(id, state) {
        ...
        // Check if this is a crash (alive changed to false with ack=true indicates system-detected crash)
        // When js-controller detects a crash, it sets alive=false with ack=true
        if (state && state.val === false && state.ack === true) {
            // Get adapter instance configuration to check if this is expected behavior
            const instanceObj = await this.adapter.getForeignObjectAsync(`system.adapter.${adapterId}`);
            
            // Skip false positives:
            // 1. Scheduled adapters (mode: 'schedule') - they stop/start on schedule, not crashes
            // 2. Disabled adapters (enabled: false) - intentionally stopped
            if (instanceObj && instanceObj.common) {
                const mode = instanceObj.common.mode;
                const enabled = instanceObj.common.enabled;
                
                if (mode === 'schedule') {
                    this.adapter.log.debug(`Ignoring alive=false for scheduled adapter: ${adapterId}`);
                    return;
                }
                
                if (enabled === false) {
                    this.adapter.log.debug(`Ignoring alive=false for disabled adapter: ${adapterId}`);
                    return;
                }
            }
    ...
    async checkForCrashes() {
        try {
            ...
            for (const [id, obj] of Object.entries(adapters)) {
                ...
                // Skip scheduled adapters (they stop/start on schedule)
                if (obj.common.mode === 'schedule') {
                    this.adapter.log.debug(`Skipping scheduled adapter in crash check: ${adapterId}`);
                    continue;
                }
                
                // Skip disabled adapters (intentionally stopped)
                if (obj.common.enabled === false) {
                    this.adapter.log.debug(`Skipping disabled adapter in crash check: ${adapterId}`);
                    continue;
                }
    ...
    async getRecentLogs(adapterId) {
        try {
            // Fetch recent log entries from the host
            const logs = await this.adapter.sendToHostAsync(this.adapter.host, 'getLog', { lines: 100 });
            
            if (!logs || !Array.isArray(logs)) {
                return [];
            }

            // Filter logs for this adapter and extract messages
            const adapterLogs = logs
                .filter(entry => entry.from && entry.from.includes(adapterId))
    ...
    async loadCrashHistory() {
        try {
            // Create state object for crash history if it doesn't exist
            await this.adapter.setObjectNotExistsAsync(`${this.adapter.namespace}.crashHistory`, {
                type: 'state',
                common: {
                    name: 'Persisted crash history',
                    type: 'string',
                    role: 'json',
                    read: true,
                    write: false
            ...
            const historyState = await this.adapter.getStateAsync('crashHistory');
            
            if (historyState && historyState.val) {
                try {
                    this.crashHistory = JSON.parse(historyState.val);
                    this.adapter.log.info(`Loaded crash history for ${Object.keys(this.crashHistory).length} adapters.`);
                } catch (parseErr) {
                    this.adapter.log.warn(`Failed to parse crash history, starting fresh: ${parseErr.message}`);
                    this.crashHistory = {};
                    await this.adapter.setStateAsync('crashHistory', JSON.stringify(this.crashHistory), true);
                }
            } else {
                this.crashHistory = {};
                await this.adapter.setStateAsync('crashHistory', JSON.stringify(this.crashHistory), true);
                this.adapter.log.debug('No existing crash history found, starting fresh.');
            }
        } catch (err) {
            ...
            try {
                ...
                this.adapter.log.error(`Failed to initialize crashHistory state: ${setErr.message}`);
            }
        }
    }

    /**
     * Save crash history to adapter state for persistence.
     */
    async saveCrashHistory() {
        try {
            await this.adapter.setStateAsync('crashHistory', JSON.stringify(this.crashHistory), true);
            this.adapter.log.debug('Crash history saved to state.');
        } catch (err) {
            this.adapter.log.error(`Failed to save crash history: ${err.message}`);
        }
    }
