...
class DuplicateStateInspector {
     ...
     * @param {Array<string>} ignorePatterns - State patterns to ignore (e.g., 'system.*', 'admin.*')
     */
    constructor(adapter, similarityThreshold = 0.9, ignorePatterns = []) {
     ...
     * @returns {boolean} True if state should be ignored
     */
    ...
    async scan() {
        ...
        try {
            ...
            for (const [id, state] of Object.entries(states)) {
                ...
                // Skip states matching ignore patterns
    ...
    async detectNamingDuplicates(stateMap) {
        ...
        for (let i = 0; i < stateArray.length; i++) {
            ...
            // Skip ignored states (already filtered in scan(), but double-check)
            if (this.shouldIgnore(baseState.id)) continue;
